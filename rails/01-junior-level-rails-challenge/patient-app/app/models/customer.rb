...
class Customer < ApplicationRecord
    ...
    def self.to_csv(customer_id, patients)
        ...
        CSV.generate(headers: true) do |csv|
            ...
            start = Time.now
            export = Export.create(customer_id: customer_id, export_status: "Complete", number_records: patients.length, created_at: start, export_duration_in_mins: ((Time.now - start)/60), export_ended: DateTime.now )
